...
static int docode(const char *begin, const char *end, int newblock); /* Parser for code blocks */
...
static Parser parsers[] = { dounderline, docomment, dolineprefix,
                            dolist, docode, doparagraph, dogtlt, dosurround, dolink,
...
int
docode(const char *begin, const char *end, int newblock) {
	const char *p;
	...
	fputs("<pre><code>\n", stdout);
	printf("%.*s", (int)(p - (begin + 4)), begin + 4);
	fputs("</code></pre>\n", stdout);
